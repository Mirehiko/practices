...
class Fixation {
  ...
  _setBlockPosition( preway ) {
    ...
    if ( this.draggH > this.winH - indentTop - indentBottom && this.draggH > this.fixedH ) {
    ...
    let params = {};
    let variant;
    if ( this.draggH < this.winH - indentTop - indentBottom ) {

      // Если фиксируемый блок целиком помещается на экране,
      // то просто фиксируем его
      if ( scroll <= upperBound - indentTop ) {

        // Если проскролили до начала родителя и выше, то прикрепляем к верхней границе родителя
        params = {
          position: 'absolute',
          top: 0,
          bottom: 'auto',
      ...
      else if ( scroll > upperBound - indentTop && scroll < this.btmBlockTop - this.draggH - indentTop ) {

        // Блок всегда виден на экране и прекреплен к верхней его части
        params = {
          position: 'fixed',
          top: indentTop,
          bottom: 'auto',
        ...
        // Если проскролили до конца родителя и ниже, то прикрепляем фиксированный блок к нижней границе родителя
        params = {
          position: 'absolute',
          top: 'auto',
          bottom: 0,
    ...
    else if ( this.draggH > this.winH - indentTop - indentBottom && this.draggH < this.fixedH ) {

      // Если фиксируемый блок не помещается на экране
      // и он меньше основного содержимого
      if ( this.scrollWay === 'down' ) {

        // Если скроллим страницу вниз
        if ( scroll < this.draggH - 10 - this.winH + upperBound + areaHigher ) {

          // где-то в пути при смене направления прокрутки
          params = {
            position: 'absolute',
            top: areaHigher,
            bottom: 'auto',
          ...
          // Если проскролили фиксированный блок до конца, то прикрепляем его к нижней границе экрана
          params = {
            position: 'fixed',
            top: 'auto',
            bottom: indentBottom,
        ...
        if ( scroll > upperBound - indentTop && scroll <= draggBlockTop - indentTop ) {

          // Пока не проскролили до верхней границы родителя прикрепляем фиксированный блок к верхней границе экрана
          params = {
            position: 'fixed',
            top: indentTop,
            bottom: 'auto',
        ...
        else if ( scroll <= upperBound - indentTop ) {
    ...
    this._setBlockProps( this.draggBlock, params );
